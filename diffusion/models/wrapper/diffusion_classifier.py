...
class DiffusionClassifier(DiffusionModel):
    ...
    def train_step(self, inputs: tuple[tf.Tensor, tf.Tensor]
        ...
        with tf.GradientTape() as tape:
            ...
            (clf_regs_list_c, clf_regs_list_u)) = self.forward(
                self.network, x_t, t, t, 
            ...
            loss1, noise_loss, image_loss, ctr_loss1, ctr_preds1 = self.compute_noise_image_ctr_loss(
                x0, noises, classes, 
                x0_pred, noises_pred, 
                regs_list_c, regs_list_u,  
            )
            loss2, clf_loss, ctr_loss2, classes_pred, ctr_preds2 = self.compute_clf_ctr_loss(
                classes, 
                classes_pred_c, 
                clf_regs_list_c, 
                classes_pred_u, 
    ...
    def test_step(self, inputs: tuple[tf.Tensor, tf.Tensor]
        ...
        loss1, noise_loss, image_loss, ctr_loss1, ctr_preds1 = super().forward_and_compute_loss(
            self.test_network, 
            x0, noises, t, x_t, 
    ...
    def compute_clf_ctr_loss(
        ...
        return loss, clf_loss, ctr_loss, classes_pred, ctr_preds
    ...
    def get_clf_results_dict(
        ...
        use_ctr_loss = self.use_ctr_loss and len(self.network.clf_cls_token_regularizer_ids) > 0 \
                    if use_ctr_loss is None else use_ctr_loss
        use_total_loss = use_ctr_loss if use_total_loss is None else use_total_loss
