...
The route/condition/stem matrix covers every pair in those factors and both
...
class JointHpoArchitectureTests(unittest.TestCase):
    """Verify executable shapes, active routes, gradients and EMA reconstruction."""

    def tearDown(self):
        tf.keras.backend.clear_session()

    def network(self, *, route="last", condition="time_label", cnn=False,
                patch=4, dim=32, heads=6, depth=3, clf_depth=1,
                mlp=None, dropout=0.0, drop_path=0.0, classes=10,
                policy="float32"):
        tf.keras.backend.clear_session()
        tf.keras.utils.set_random_seed(1729)
        overrides = {
            "aggregate_from_noises": [route == "noise"],
            "feature_aggregation": ["all" if route == "all" else "last"],
            "clf_cond_type": [condition], "patchify_with_cnn": [cnn],
    ...
    def check_shapes_and_routes(self, network, config, inputs):
        ...
        if not network.aggregate_from_noises:
        ...
        self.assertEqual(len(dense), 1 if network.classifier_mlp_ratio is None else 2)
        if network.classifier_mlp_ratio is not None:
    ...
    def test_route_condition_stem_matrix_has_trainable_image_connected_classifier(self):
        """Cover 24 route/condition/CNN triples and both patch/dropout choices."""
        conditions = (None, "time", "label", "time_label")
        for route_index, route in enumerate(("last", "all", "noise")):
            for condition_index, condition in enumerate(conditions):
                for cnn in (False, True):
                    case = route_index * 8 + condition_index * 2 + int(cnn)
                    options = dict(
                        route=route, condition=condition, cnn=cnn,
                        patch=2 if (condition_index + int(cnn)) % 2 else 4,
                        heads=(4, 6, 8)[case % 3], depth=3 + case % 5,
                        clf_depth=1 + (case // 2) % 5,
                        mlp=(None, 1, 2, 4)[condition_index],
                        dropout=0.25 if cnn else 0.0,
                        drop_path=0.25 if condition_index % 2 else 0.0,
        ...
        for case, (dim, heads) in enumerate(itertools.product((32, 64, 128, 256), (4, 6, 8))):
            with self.subTest(dim=dim, heads=heads):
                network, config = self.network(
                    dim=dim, heads=heads, route=("last", "all", "noise")[case % 3],
                    patch=2 if case % 2 else 4, cnn=bool(case % 2),
                    condition=(None, "time", "label", "time_label")[case % 4],
                    depth=7 if dim == 256 else 3, clf_depth=5 if dim == 256 else 1,
                    mlp=4 if dim == 256 else None, classes=100,
                )
                self.check_shapes_and_routes(network, config, self.inputs())

    def test_mixed_precision_graph_and_ema_clone_preserve_each_route(self):
        """Cover six route/stem pairs using the notebook's bfloat16 policy."""
        for route, cnn in itertools.product(("last", "all", "noise"), (False, True)):
            with self.subTest(route=route, cnn=cnn):
                network, config = self.network(
                    route=route, cnn=cnn, condition="time_label" if cnn else None,
                    patch=2 if cnn else 4, policy="mixed_bfloat16", classes=100,
                    dropout=0.25, drop_path=0.25, mlp=2,
                )
                inputs = self.inputs()
                self.check_shapes_and_routes(network, config, inputs)
                self.check_joint_gradients_and_image_dependence(network, inputs)
                ...
                for name in ("noises", "classes"):
                    np.testing.assert_allclose(actual[name], expected[name], rtol=0.025, atol=0.003)
                tail = graph(*(value[:1] for value in inputs))
                for name in ("noises", "classes"):
                    np.testing.assert_allclose(tail[name], actual[name][:1], rtol=0.025, atol=0.003)
                ...
                self.assertEqual(clone.dtype_policy.name, "mixed_bfloat16")
    ...
    def test_head_dropout_and_classifier_drop_path_are_independently_active(self):
        ...
        for dropout, drop_path in ((0.25, 0.0), (0.0, 0.25)):
