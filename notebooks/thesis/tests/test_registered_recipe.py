    ...
    def test_resolved_ensemble_options_construct_with_real_two_head_wrapper(self) -> None:
        ...
        for dataset in ("cifar10", "cifar100"):
            with self.subTest(dataset=dataset):
                ...
                self.assertEqual(metric.t_range_drop_rate, .75)
                self.assertEqual(metric.clf_acc_coef, 1.)
                self.assertEqual(metric.clf_distil_acc_coef, 0.)
    ...
    def test_cosine_horizon_covers_replay_growth_and_fixed_extra_joint(self) -> None:
        ...
        for dataset in ("cifar10", "cifar100"):
            with self.subTest(dataset=dataset):
                ...
                self.assertAlmostEqual(actual[0], .001, places=8)
